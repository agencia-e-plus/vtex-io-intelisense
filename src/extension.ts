...
export function activate(context: vscode.ExtensionContext) {
	...
	const command = vscode.commands.registerCommand('vtexiointellisense.storeLint', async () => {
	...
	const provider1 = vscode.languages.registerCompletionItemProvider('jsonc', {
	...
	const provider2 = vscode.languages.registerCompletionItemProvider(
		'jsonc',
		{
			provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
				...
				// const { allJSONs } = FileRegister.getInstance()
				const allJSONs = {}

				return Object.keys(allJSONs).map(
					(key) => new vscode.CompletionItem(`"${key}"`, vscode.CompletionItemKind.Method)
				)
			}
		},
	...
	context.subscriptions.push(blocksDiagnostics, provider1, provider2)
